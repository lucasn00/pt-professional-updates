...
def fetch_items_from_feeds(label, urls):
    seen_titles = set()
    items = []
    for url in urls:
        try:
            ...
            print(f"  Warning: Error fetching [{label}] from {url}: {e}")
    return items
...
def build_message(all_items):
    ...
    for label, items in all_items.items():
        ...
        if not items:
            lines.append("  No new updates this week for this topic.")
        else:
            any_content = True
            for item in items:
    ...
    if not any_content:
        lines.append("No new updates found across all topics this week. Check back next Monday!\n")
    ...
    lines += [
        ...
        "PTJ: https://academic.oup.com/ptj",
        "PubMed: https://pubmed.ncbi.nlm.nih.gov/?term=physical+therapy",
